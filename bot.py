...
TASK_REWARD = 10   # Daily Task সম্পন্নে পয়েন্ট
MAX_ADS_PER_DAY = 10  # দিনে সর্বোচ্চ কতটি Ad দেখা যাবে
...
def register_user(user_id, username, full_name, referred_by=None):
    ...
    if not c.fetchone():
        c.execute(
            "INSERT INTO users (user_id, username, full_name, referred_by, joined_at) VALUES (?,?,?,?,?)",
...
def add_ad_watch(user_id):
    ...
    if ads >= MAX_ADS_PER_DAY:
        ...
        return False
    c.execute(
        "INSERT OR REPLACE INTO daily_stats (user_id, stat_date, ads_watched, task_done) VALUES (?,?,?,?)",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu":
        await query.edit_message_text(
            f"🏠 *মেইন মেনু*\n\nস্বাগতম, {user.first_name}! কি করতে চান?",
            parse_mode="Markdown",
            reply_markup=main_menu_keyboard(user.id)
        )
        ...
        if remaining <= 0:
            await query.edit_message_text(
                "⏰ *আজকের Ad সীমা শেষ!*\n\n"
                f"আপনি আজ সর্বোচ্চ {MAX_ADS_PER_DAY}টি Ad দেখেছেন।\n"
                "কাল আবার আসুন! 🌅",
                parse_mode="Markdown",
        ...
        ad_url = f"{WEB_APP_URL}/ad?user={user.id}&token={user.id}_{date.today().isoformat()}"
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📺 Ad দেখুন (নতুন ট্যাব)", url=ad_url)],
        ...
        await query.edit_message_text(
            f"📺 *Ad দেখুন*\n\n"
            f"আজ বাকি: *{remaining}টি* Ad\n"
            f"প্রতিটি Ad: *{AD_REWARD} টাকা*\n\n"
            "👇 নিচের বাটনে ক্লিক করে Ad দেখুন,\n"
            "তারপর '✅ Ad দেখা হয়েছে' বাটন চাপুন।",
            parse_mode="Markdown",
        ...
        if success:
            ...
            await query.edit_message_text(
                ...
                f"💰 আপনি পেয়েছেন: *{AD_REWARD} টাকা*\n"
                f"📺 আজ বাকি: *{remaining}টি* Ad\n\n"
                "আরো দেখতে চান? মেনুতে ফিরুন।",
                parse_mode="Markdown",
                reply_markup=back_keyboard()
            )
        else:
            await query.edit_message_text(
                "⏰ আজকের Ad সীমা শেষ হয়ে গেছে!",
                reply_markup=back_keyboard()
            )
        ...
        if task_done:
            await query.edit_message_text(
                ...
                f"আপনি আজ {TASK_REWARD} টাকা পেয়েছেন।\n"
                "কাল আবার আসুন! 🌅",
                parse_mode="Markdown",
                reply_markup=back_keyboard()
            )
        ...
        if success:
            await query.edit_message_text(
                ...
                f"💰 আপনি পেয়েছেন: *{TASK_REWARD} টাকা*\n"
                "কাল আবার আসতে ভুলবেন না! 😊",
                parse_mode="Markdown",
                reply_markup=back_keyboard()
            )
        else:
            await query.edit_message_text(
                "✅ আজকের Task আগেই সম্পন্ন হয়েছে!",
                reply_markup=back_keyboard()
            )
        ...
        await query.edit_message_text(
            ...
            f"💰 প্রতি Refer: *৫ টাকা*\n\n"
            "🔗 বন্ধুদের সাথে শেয়ার করুন এবং আয় বাড়ান!",
            parse_mode="Markdown",
            reply_markup=back_keyboard()
        )
        ...
        await query.edit_message_text(
            f"💰 *আপনার Balance*\n\n"
            f"💵 বর্তমান Balance: *{balance:.0f} টাকা*\n"
            f"📈 মোট আয়: *{total:.0f} টাকা*\n\n"
        ...
        if balance < MIN_WITHDRAW:
            await query.edit_message_text(
                ...
                f"আপনার Balance: *{balance:.0f} টাকা*\n"
                f"Minimum দরকার: *{MIN_WITHDRAW} টাকা*\n\n"
        ...
        await query.edit_message_text(
            f"📊 *আপনার Stats*\n\n"
            f"💵 Balance: *{balance:.0f} টাকা*\n"
            f"📈 মোট আয়: *{total:.0f} টাকা*\n\n"
            f"📅 *আজকের রিপোর্ট:*\n"
            f"📺 Ad দেখেছেন: *{ads_today}/{MAX_ADS_PER_DAY}টি*\n"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_withdraw"):
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO withdrawals (user_id, amount, method, number, requested_at) VALUES (?,?,?,?,?)",
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"💰 Total Balance (সব ইউজার): *{total_balance:.0f} টাকা*",
        parse_mode="Markdown",
        reply_markup=keyboard
    )

async def approve_withdrawal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Command: /approve <withdrawal_id>"""
    if update.effective_user.id != ADMIN_ID:
        return
    if not context.args:
    ...
    wd = c.fetchone()
    if not wd:
        await update.message.reply_text("❌ Withdrawal পাওয়া যায়নি।")
        conn.close()
        return
    ...
    try:
        await context.bot.send_message(
            wd[1],
            f"🎉 *আপনার Withdrawal Approved!*\n\n"
            f"💰 Amount: *{wd[2]:.0f} টাকা*\n"
            f"📱 {wd[3]}: `{wd[4]}`\n\n"
            "পেমেন্ট পাঠানো হয়েছে। ধন্যবাদ! 🙏",
            parse_mode="Markdown"
        )
    except:
        pass
